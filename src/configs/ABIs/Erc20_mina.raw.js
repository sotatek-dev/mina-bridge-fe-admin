...
export class Token extends SmartContract {
  ...
  approveCallbackAndTransfer(sender, receiver, amount, callback) {
    ...
    const senderAccountUpdate = this.approve(
      callback,
      AccountUpdate.Layout.AnyChildren
    );
    senderAccountUpdate.body.tokenId.assertEquals(tokenId);
    senderAccountUpdate.body.publicKey.assertEquals(sender);
    const negativeAmount = Int64.fromObject(
      senderAccountUpdate.body.balanceChange
    );
    negativeAmount.assertEquals(Int64.from(amount).neg());
    const receiverAccountUpdate = Experimental.createChildAccountUpdate(
      this.self,
      receiver,
      tokenId
    );
...
__decorate(
  [
    method,
    __metadata('design:type', Function),
    __metadata('design:paramtypes', [PublicKey, UInt64, Experimental.Callback]),
    __metadata('design:returntype', void 0),
  ],
  Token.prototype,
  'sendTokensFromZkApp',
